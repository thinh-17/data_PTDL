...
def main():
    ...
    engine = get_engine()
    df = pd.read_sql("SELECT * FROM storage.mart_seller_cluster_features", engine)

    id_cols = ["seller_key", "seller_id"]
    ...
    result.to_csv(OUTPUT_DIR / "seller_clusters.csv", index=False)
    ...
    summary.to_csv(OUTPUT_DIR / "seller_cluster_summary.csv")

    fig, ax = plt.subplots(figsize=(7, 4))
    summary["total_revenue"].plot(kind="bar", ax=ax)
    ax.set_title("Average Revenue by Seller Cluster")
    ax.set_xlabel("Cluster")
    ax.set_ylabel("Average total revenue")
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / "seller_cluster_total_revenue.png")
    plt.close(fig)

    joblib.dump(pipeline, OUTPUT_DIR / "seller_kmeans_model.pkl")
